...
export default function ShapeOnTerms() {
    const app = apps.find(a => a.id === "shapeon");
    const document = app?.documents.terms;

    if (!document) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white font-black uppercase tracking-tighter">
                <div className="text-center">
                    <h1 className="text-2xl font-bold mb-2">Access Denied</h1>
                    <p className="text-slate-500 mb-6">Terms of Service encryption failure.</p>
    ...
    return (
        <div className="min-h-screen bg-slate-950 font-sans text-slate-300 selection:bg-lime-400 selection:text-black">
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 py-20 md:py-32 relative">
                ...
                <div className="absolute top-0 left-0 w-64 h-64 bg-emerald-500/5 rounded-full blur-[100px] -z-10"></div>
                
                <div className="mb-20 border-l-4 border-emerald-400 pl-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-6 text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                        <p>Established: <span className="text-slate-300">{document.lastUpdated}</span></p>
                        <span className="hidden sm:inline w-1 h-1 bg-slate-800 rounded-full"></span>
                        <p>Channel: <a href={`mailto:${document.contactEmail}`} className="text-emerald-400 hover:text-emerald-300 transition-colors">{document.contactEmail}</a></p>
                    </div>
                </div>

                <div className="grid grid-cols-3 gap-4 mb-20">
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800/50">
                        <Scale className="w-5 h-5 text-emerald-400 mb-3" />
                        <div className="text-white text-xs font-bold uppercase mb-1 tracking-tighter">Fair Play</div>
                        <div className="text-[10px] text-slate-500 font-medium leading-tight">Balanced user rights</div>
                    </div>
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800/50">
                        <FileCheck className="w-5 h-5 text-emerald-400 mb-3" />
                        <div className="text-white text-xs font-bold uppercase mb-1 tracking-tighter">Compliance</div>
                        <div className="text-[10px] text-slate-500 font-medium leading-tight">Bio-regulatory standards</div>
                    </div>
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800/50">
                        <ArrowLeft className="w-5 h-5 text-emerald-400 mb-3 rotate-45" />
                        <div className="text-white text-xs font-bold uppercase mb-1 tracking-tighter">Agility</div>
                        <div className="text-[10px] text-slate-500 font-medium leading-tight">Adaptive legal frameworks</div>
                    </div>
                </div>

                <div
                    className="prose prose-invert prose-emerald max-w-none text-slate-400 font-medium leading-relaxed
                    ...
                    dangerouslySetInnerHTML={{
                        __html: document.content
                            .replace(/<h2>/g, '<h2 class="text-3xl font-black text-white mt-24 mb-10 border-b border-white/10 pb-4 uppercase italic tracking-tight text-emerald-400">')
            ...
            <footer className="bg-slate-900/30 border-t border-slate-900 py-16">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-8">
                    ...
                    <div className="flex gap-8 text-[10px] font-black uppercase tracking-[0.2em]">
                        <Link href="/shapeon/privacy" className="text-slate-500 hover:text-emerald-400 transition-colors">
                            Privacy Node
                        </Link>
                        <Link href="/shapeon" className="text-slate-500 hover:text-emerald-400 transition-colors">
                            Main Interface
                        </Link>
                    </div>
                </div>
